    ...
    >
      ...
      >
        ...
        <div className={`relative w-[50px] h-[50px] flex flex-col items-center justify-center rounded-badge overflow-hidden ${badgeClass[day.type]}`}>
          <span className="font-display text-xl font-bold text-white leading-none relative z-10">
            {day.num}
          </span>
        ...
        <div className="min-w-0 text-left">
          <p className="font-mono text-[0.58rem] tracking-[0.08em] uppercase text-smoke-light">
            {day.date}
          </p>
          <h3 className="font-display text-[1.1rem] font-semibold text-volcanic leading-tight truncate">
            {day.title}
          </h3>
        </div>
        ...
        <div className="hidden sm:flex gap-1.5 flex-shrink-0 flex-wrap justify-end">
          {day.tags.map((tag) => (
            <span
              key={tag}
              className={`font-mono text-[0.52rem] tracking-[0.06em] uppercase px-2.5 py-1 rounded-full whitespace-nowrap font-medium ${tagStyles[tag] || 'bg-smoke/10 text-smoke'}`}
            >
              {tag}
            </span>
          ))}
        ...
        <motion.div
          ...
          className={`
            flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-full transition-colors duration-300
            ${isOpen ? 'bg-volcanic' : 'bg-sand'}
          `}
        >
